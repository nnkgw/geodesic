...
// Methods: Dijkstra (1-skeleton), Selective Refinement (iterative, ONFACE rungs)
// Libraries: freeglut, OpenGL (fixed pipeline), glm
// Usage: ./app model.obj
//
// Keys:
//   1 / 2 : choose Dijkstra / Selective Refinement
//   Space : overlay both paths
//   O     : toggle SR debug overlay (steiner points / refined chains / onface / SR polyline)
//   [ / ] : decrease / increase m (Steiner points per subdivided edge)
//   P     : toggle ONFACE rung edges on/off
//   , / . : decrease / increase eps (relative threshold)
//   9 / 0 : decrease / increase itmax (max iterations)
//   R     : re-sample random endpoints
//   + / - : zoom in / out
//   Mouse : L-rotate, R-pan, wheel zoom (freeglut)
//   ESC   : quit
...
static SRParams gSRP{};          // m / eps / itmax / addOnFace
static bool gShowSRDebug = false; // overlay steiner points, refined chains, onface rungs, SR polyline
...
// debug overlays from SR (final iteration)
static std::vector<glm::vec3> dbgSteinerPts; // cyan points
static std::vector<std::pair<glm::vec3, glm::vec3>> dbgRefinedEdges; // cyan segments
static std::vector<std::pair<glm::vec3, glm::vec3>> dbgOnFaceEdges;  // magenta segments
static std::vector<glm::vec3> dbgSRPolyline; // yellow polyline (includes Steiner nodes)
  ...
  while(std::getline(ifs,line)){
    ...
    if(tag=="v"){
      ...
      auto readIndex = [&](const std::string& tok)->int{
        ...
        if(a.empty()) return -1;
        int idx = std::stoi(a);
        if(idx<0) idx = (int)verts.size()+idx+1; // OBJ negative index support
...
static void recomputePath(){
  ...
  // Selective Refinement (iterative, ONFACE, with debug outputs from final iteration)
  pathSR.clear();
  dbgSteinerPts.clear();
...
static void keyboard(unsigned char key,int,int){
  switch(key){
    ...
    // Change SR parameter m (Steiner points per subdivided edge)
    case '[':
      if (gSRP.m > 1) { gSRP.m--; recomputePath(); glutPostRedisplay(); }
      break;
    case ']':
      if (gSRP.m < 32) { gSRP.m++; recomputePath(); glutPostRedisplay(); }
      break;
    ...
    // Tune eps (relative threshold)
    case ',':
      gSRP.eps *= 0.5f;
      if (gSRP.eps < 1e-6f) gSRP.eps = 1e-6f;
      recomputePath(); glutPostRedisplay(); break;
    case '.':
      gSRP.eps *= 2.0f;
      if (gSRP.eps > 1e-1f) gSRP.eps = 1e-1f;
      recomputePath(); glutPostRedisplay(); break;

    // Tune itmax
    case '9':
      if (gSRP.itmax > 1) gSRP.itmax--;
      recomputePath(); glutPostRedisplay(); break;
    case '0':
      if (gSRP.itmax < 50) gSRP.itmax++;
      recomputePath(); glutPostRedisplay(); break;
...
static void usage(){
  ...
  std::puts("  2           : select Selective Refinement (iterative)");
  ...
  std::puts("  , / .       : decrease / increase eps (relative threshold)");
  std::puts("  9 / 0       : decrease / increase itmax (max iterations)");
  std::puts("  R           : re-sample random endpoints");
  std::puts("  + / -       : zoom in / out");
  std::puts("  ESC         : quit");

  std::printf("\nCurrent SR params: m=%d, eps=%.3g, itmax=%d, onface=%s\n",
              gSRP.m, gSRP.eps, gSRP.itmax, gSRP.addOnFace ? "on" : "off");
  std::puts("");
}
...
int main(int argc,char** argv){
  ...
  // Default SR parameters (visible & reasonable)
  gSRP.m = 2;        // steiner per subdivided edge
  gSRP.eps = 1e-3f;  // relative threshold for convergence
  gSRP.itmax = 10;   // max iterations
  gSRP.addOnFace = true;
  ...
  glutCreateWindow("Shortest Path on Mesh (Dijkstra & SR Iterative)");
